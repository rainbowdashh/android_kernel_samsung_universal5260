	...
	struct fimc_is_module_enum *active_sensor)
{
	...
	if (checksum != buf32[check_base + 1023]) {
		merr("header checksum is invalid(0x%08X != 0x%08X)",
			device, checksum, buf32[check_base + 1023]);
		check_ok = false;
	}
...
int fimc_is_ischain_power(struct fimc_is_device_ischain *device, int on)
{
	...
	if (on) {
		...
		if (fimc_is_sec_fw_module_compare(FW_3L2, finfo->header_ver))
			cam_sensor = SENSOR_NAME_S5K3L2;
		else if (fimc_is_sec_fw_module_compare(FW_IMX135, finfo->header_ver))
			cam_sensor = SENSOR_NAME_IMX135;
		else if (fimc_is_sec_fw_module_compare(FW_IMX134, finfo->header_ver))
			cam_sensor = SENSOR_NAME_IMX134;
		else if (fimc_is_sec_fw_module_compare(FW_IMX175, finfo->header_ver))
			cam_sensor = SENSOR_NAME_IMX175;
		else
			cam_sensor = SENSOR_NAME_IMX175;

		fimc_is_sensor_find_module(sensor, cam_sensor, &module);

		if ((cam_id == CAMERA_SINGLE_REAR || cam_id == CAMERA_DUAL_FRONT) && module) {
			if (fimc_is_sec_fw_module_compare(FW_3L2, finfo->header_ver)) {
				info("AF actuator is WV560\n");
				module->ext.actuator_con.product_name = ACTUATOR_NAME_WV560;
			} else if (fimc_is_sec_fw_module_compare(FW_IMX135, finfo->header_ver)) {
				info("AF actuator is WV560\n");
				module->ext.actuator_con.product_name = ACTUATOR_NAME_WV560;
			} else if (fimc_is_sec_fw_module_compare(FW_IMX134, finfo->header_ver)) {
				info("AF actuator is AK7345\n");
				module->ext.actuator_con.product_name = ACTUATOR_NAME_AK7345;
			} else if (fimc_is_sec_fw_module_compare(FW_IMX175, finfo->header_ver)) {
				info("AF actuator is AK7343 - default PID = %d\n",
						fimc_is_sec_get_ak7343_default_pid());
				module->ext.actuator_con.product_name = ACTUATOR_NAME_AK7343;

				module->ext.actuator_con.reserved[0] = 1;
				module->ext.actuator_con.reserved[1] = (u32) fimc_is_sec_get_ak7343_default_pid();
			} else {
				info("AF actuator is AK7343\n");
				module->ext.actuator_con.product_name = ACTUATOR_NAME_AK7343;
			}
		}
	...
	struct camera2_node *node)
{
	...
	if (!GET_FIMC_IS_NUM_OF_SUBIP2(device, 3a0)) {
		...
		if (IS_EQUAL_COORD(input_crop, output_crop)) {
			ret = fimc_is_ischain_s_3aa_size(device,
				ldr_frame,
				input_crop,
				output_crop,
